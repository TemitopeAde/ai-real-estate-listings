...
export function usablePriceRange(range: ListingPriceRange | null, listings: Array<{ price: number; currency: string }>): ListingPriceRange {
  ...
  const minPrice = range?.minPrice ?? listed?.minPrice ?? FALLBACK_PRICE_RANGE.minPrice;
  ...
  return { minPrice: 0, maxPrice: Math.max(minPrice * 2, FALLBACK_PRICE_RANGE.maxPrice), currency };
}
